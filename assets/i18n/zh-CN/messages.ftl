...
command-rename-window = 重命名窗口
palette-window-name-placeholder = 窗口名称（留空重置）…
palette-window-name-footer = ↵ 保存 · 留空重置 · esc 取消
palette-window-name-controls = 名称不能包含控制字符或换行
palette-window-name-too-long = 名称不能超过 128 个字符
...
palette-about-title = 关于 SonicTerm
palette-about-search = 关于 SonicTerm 版本 构建
