// Modified by SonicTerm contributors to expose owned native Windows keyboard metadata.
//! # Windows
//!
...
/// Native Windows message metadata associated with one delivered keyboard event.
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct NativeKeyEvent {
    /// Unmodified virtual-key value from the key message's `wParam`.
    pub virtual_key: u16,
    /// Scan-code byte from `lParam`, without an extended-key prefix.
    pub scan_code: u16,
    /// Windows console control-key-state flags captured before modifier callbacks.
    pub control_key_state: u32,
    /// Whether the native transition-state bit describes a key press.
    pub key_down: bool,
    /// Unmodified low-word repeat count from the native key message.
    pub repeat_count: u16,
    /// Associated UTF-16 units, including unmatched surrogates; empty for key releases.
    pub unicode: Vec<u16>,
}

/// Windows-specific access to the native metadata carried by a keyboard event.
pub trait KeyEventExtWindows {
    /// Returns native metadata, or `None` for synthetic events or unavailable snapshots.
    fn native_key_event(&self) -> Option<&NativeKeyEvent>;
}

impl KeyEventExtWindows for KeyEvent {
    fn native_key_event(&self) -> Option<&NativeKeyEvent> {
        self.platform_specific.native_key_event.as_ref()
    }
}
