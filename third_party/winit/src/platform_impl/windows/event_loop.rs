// Modified by SonicTerm contributors to snapshot native key state before modifier callbacks.
...
use windows_sys::Win32::UI::Input::KeyboardAndMouse::{
    GetKeyboardState, ReleaseCapture, SetCapture, TrackMouseEvent, TME_LEAVE, TRACKMOUSEEVENT,
};
...
use crate::platform_impl::platform::keyboard::{native_key_event, KeyEventBuilder};
...
unsafe fn public_window_callback_inner(
    ...
    let native_key_event = match msg {
        // When: native key messages must snapshot keyboard_state before modifier callbacks can reenter.
        WM_KEYDOWN | WM_SYSKEYDOWN | WM_KEYUP | WM_SYSKEYUP => {
            let mut keyboard_state = [0; 256];
            // SAFETY: GetKeyboardState writes exactly 256 bytes into the live snapshot buffer.
            if unsafe { GetKeyboardState(keyboard_state.as_mut_ptr()) } != 0 {
                Some(native_key_event(wparam, lparam, &keyboard_state))
            // When: GetKeyboardState fails, None prevents publishing a fabricated all-up snapshot.
            } else {
                None
            }
        },
