// Modified by SonicTerm: retain desktop documentation without the upstream changelog.
...
//!
//! * `x11` (enabled by default): On Linux, enables the X11 backend.
//! * `wayland` (enabled by default): On Linux, enables the Wayland backend.
...
//! [^1]: `EventLoopExtPumpEvents::pump_app_events()` is only available on Windows, macOS, X11 and Wayland.
