// Modified by SonicTerm contributors to preserve native keyboard metadata through text delivery.
use std::char;
...
use crate::platform::windows::NativeKeyEvent;
...
/// Captures native key-message fields using the callback's unchanged keyboard snapshot.
pub(crate) fn native_key_event(
    wparam: WPARAM,
    lparam: LPARAM,
    keyboard_state: &[u8; 256],
) -> NativeKeyEvent {
    // KEY_EVENT_RECORD control flags are independent of winit's AltGr modifier normalization.
    ...
    for (vk, mask, flag) in [
        (VK_RMENU, 0x80, RIGHT_ALT_PRESSED),
        (VK_LMENU, 0x80, LEFT_ALT_PRESSED),
        (VK_RCONTROL, 0x80, RIGHT_CTRL_PRESSED),
        (VK_LCONTROL, 0x80, LEFT_CTRL_PRESSED),
        (VK_SHIFT, 0x80, SHIFT_PRESSED),
        (VK_NUMLOCK, 1, NUMLOCK_ON),
        (VK_SCROLL, 1, SCROLLLOCK_ON),
        (VK_CAPITAL, 1, CAPSLOCK_ON),
    ] {
        // When: each flag uses its own keyboard_state byte, preserving side and toggle distinctions.
        if keyboard_state[vk as usize] & mask != 0 {
            control_key_state |= flag;
        }
    }
    // When: lparam's extended-key bit belongs in control_key_state, not the scan-code byte.
    if lparam & (1 << 24) != 0 {
        control_key_state |= ENHANCED_KEY;
    }
    NativeKeyEvent {
        virtual_key: wparam as u16,
        scan_code: ((lparam >> 16) & 0xff) as u16,
        control_key_state,
        key_down: lparam & (1 << 31) == 0,
...
impl PartialKeyEventInfo {
    ...
    fn finalize(self) -> KeyEvent {
        // Preserve native UTF-16 before portable text conversion can reject unmatched surrogates.
        let native_key_event = self.native_key_event.map(|mut native| {
            // When: only key_down can own utf16parts; releases must never replay press text.
            if native.key_down {
                native.unicode = self.utf16parts.clone();
            }
            native
        });
