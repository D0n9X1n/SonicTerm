...
//! - `sonicterm-grid` is added as a third dep (alongside `sonicterm-types`
//!   and `sonicterm-cfg`) because `selection` and `search` need
//!   `Grid`/`Cell`/`CellFlags`. `sonicterm-grid` is itself pure data
