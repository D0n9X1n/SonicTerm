//! Pure-data application state owned by the backend-free reducer.
//!
//! Platform and renderer state remain in `sonicterm-app`; this module records
//! only the deterministic state needed to reduce intents into effects.

use sonicterm_types::WindowKey;

use crate::supporting::{BroadcastScope, LogicalPos};

/// Pure-data application state used by deterministic reducer transitions.
///
/// Window focus, grid dimensions, logical position, and topology counters let
/// the reducer suppress no-op effects without owning platform windows, panes,
/// tabs, PTYs, or renderers.
...
pub struct AppState {
    ...
    /// by the reducer arms and the focused-tab tests in `tab_intents.rs`.
    /// Updates saturate in both directions.
    pub tab_count: u32,
    ...
    /// remains source-of-truth for actual pane content; this counter is the
    /// observability surface used by reducer arms and `pane_intents.rs` tests.
    pub pane_count: u32,
    ...
    /// path remains source-of-truth for the rendered layout; this boolean
    /// tracks the reducer's observable view of the toggle.
    ...
    /// boundary's `WindowState.mouse_down` remains source-of-truth for actual
    /// hit-test gates; this flag is the reducer's observability surface.
    pub mouse_left_down: bool,
    /// Whether the search overlay is open. Toggled by `OpenSearch` /
    /// `CloseSearch`. Reducer emits `Render(Overlay)` only on the
    /// open/close transition, matching the `WindowFocused` transition guard.
    ...
    /// `ClearSelection`. The boundary's per-window `selection` remains
    /// source-of-truth for the rendered rectangle; this is observability state.
    pub selection_active: bool,
    /// Last foreground-process name observed for the focused pane.
    /// `None` until the first `ForegroundProcChanged`; title rendering occurs
    /// only when the observed name changes.
    ...
    /// `App.broadcast` remains source-of-truth for actual fan-out; the reducer
    /// flag gates title and tab-strip repainting.
...
/// Builder for the initial grid dimensions of [`AppState`].
#[derive(Debug, Default)]
