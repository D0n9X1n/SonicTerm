...
//! - Per-pane replay buffer (256 KiB ring) delivered as bounded, self-delimiting
//!   fragments through an explicit parser-reset/replay handshake after attach or
//!   a live-stream gap.
//!
...
/// Length-prefixed bincode framing over the mux socket (`read_frame`/`write_frame`).
pub mod frame;
...
pub use server::{
    handle_connection, handle_connection_with_shutdown, OutputSendResult, ServerState,
    SubscriberSink, CHANNEL_CAP, REPLAY_CAP,
};
