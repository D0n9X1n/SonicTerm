...
impl HoveredUrl {
    /// Project this hover hit into the renderer-facing
    /// [`sonicterm_render_model::inputs::HoveredUrlCells`] used to
    /// recolor the URL's glyphs with the theme accent. Drops the `url`
    /// string and keeps the viewport cell range verbatim (`row`,
    /// `start_col` inclusive, `end_col` exclusive) — the same
    /// coordinates the underline projection consumes, so recolor and
    /// underline always cover identical cells.
    #[must_use]
...
#[cfg(test)]
#[path = "../../tests/hovered_url_tests.rs"]
mod tests;
