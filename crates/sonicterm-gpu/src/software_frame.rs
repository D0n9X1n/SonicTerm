//! Platform-neutral CPU frame composition, sampling, and color conversion.
//!
//! Windows production and every host's unit tests compile this module. Native
//! window handles and GDI presentation live exclusively in `software_windows`;
//! compiling the compositor for tests does not add a non-Windows presenter.
...
/// Platform-neutral CPU compositor with a bounded, retained BGRA pixel buffer.
...
/// Borrowed native-blit view whose extent and initialized storage share one frame owner.
#[cfg(target_os = "windows")]
pub(crate) struct BgraFrame<'a> {
    frame: &'a SoftwareFrame,
}

#[cfg(target_os = "windows")]
impl BgraFrame<'_> {
    /// Width of the borrowed frame in pixels.
    pub(crate) fn width(&self) -> u32 {
        self.frame.width
    }

    /// Height of the borrowed frame in pixels.
    pub(crate) fn height(&self) -> u32 {
        self.frame.height
    }

    /// Initialized BGRA bytes covering exactly the borrowed frame's extent.
...
impl SoftwareFrame {
    /// Allocate a validated CPU frame and paint its initial background.
    ...
    /// Validate a resize, retain or shrink storage, and repaint the whole background.
    ...
    /// Compose the five frame layers with grayscale text coverage for pixel tests.
    ...
    /// Compose base quads, images, glyphs, then overlays with the resolved text AA policy.
    #[allow(clippy::too_many_arguments)]
    ...
    /// Borrow the validated extent and initialized top-down BGRA storage as one native-blit view.
    #[cfg(target_os = "windows")]
    pub(crate) fn bgra_frame(&self) -> BgraFrame<'_> {
        BgraFrame { frame: self }
    }

    /// Return one composed BGRA pixel, rejecting coordinates outside the frame.
...
#[cfg(test)]
#[path = "software_frame_tests.rs"]
mod software_frame_tests;
