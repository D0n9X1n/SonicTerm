...
impl CopyModeState {
    pub fn new_at(pos: (usize, usize)) -> Self {
        Self { cursor: pos, anchor: None, mode: CopyMode::Cursor }
    }
...
fn max_row(grid: &Grid) -> usize {
    grid.rows.saturating_sub(1) as usize
}

fn visible_row(grid: &Grid, row: usize) -> Option<&Row> {
    (row < grid.rows as usize).then(|| grid.row(row as u16))
}
...
fn next_pos(grid: &Grid, pos: (usize, usize)) -> Option<(usize, usize)> {
    ...
    if col < grid.cols as usize {
        Some((col, pos.1))
    } else if pos.1 + 1 < grid.rows as usize {
        Some((0, pos.1 + 1))
    } else {
        None
    }
