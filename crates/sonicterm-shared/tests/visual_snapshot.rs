...
//!
//! Workflow:
//!   `cargo test -p sonicterm-shared --test visual_snapshot` — assert.
//!   `UPDATE_SNAPSHOTS=1 cargo test -p sonicterm-shared --test visual_snapshot`
...
// `mono_alpha_byte_layout` in sonicterm-text. Baselines refreshed on
