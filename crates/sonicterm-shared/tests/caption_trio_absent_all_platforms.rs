...
//! (`crates/sonicterm-shared/src/render/core.rs`), the GPU helper module
//! (`crates/sonicterm-gpu/src/quad.rs`), and the UI tab-bar view
//! (`crates/sonicterm-ui/src/tabbar_view.rs`). If any of them re-introduces
...
fn ui_tabbar_view_does_not_export_caption_button_helpers() {
    let source = include_str!("../../sonicterm-ui/src/tabbar_view.rs");
    assert!(
        !source.contains("caption_button_rects"),
        "sonicterm-ui::tabbar_view must not export caption_button_rects (#366)"
    );
    assert!(
        !source.contains("CAPTION_BUTTON_WIDTH"),
        "sonicterm-ui::tabbar_view must not export CAPTION_BUTTON_WIDTH (#366)"
    );
    assert!(
        !source.contains("caption_strip_reserved_width"),
        "sonicterm-ui::tabbar_view must not export caption_strip_reserved_width (#366)"
    );
}
