...
//! `sonic-mac::tab_drag_os::MacOsTabDragBackend::begin_session` and
...
use sonic_app::app::os_drag::{AppHandle, DragOutcome, OsTabDragBackend};
...
fn mock_backend_records_call_when_invoked_directly() {
    ...
    let proxy_opt = std::panic::catch_unwind(|| {
        winit::event_loop::EventLoop::<sonic_app::app::UserEvent>::with_user_event()
            .build()
            .ok()
