...
fn drag_session_accessible_via_main() {
    ...
    if let Some(ws) = app.main_mut() {
        ws.drag_session = Some(sonic_app::tab_drag::DragSession::new(2, (10.0, 20.0)));
    }
...
fn drag_target_accessible_via_main() {
    ...
    app.__test_set_drag_target(Some(sonic_app::tab_drag::DropTarget { window: main_id, slot: 1 }));
...
fn multi_window_owns_independent_drag_session() {
    ...
    a.drag_session = Some(sonic_app::tab_drag::DragSession::new(0, (1.0, 2.0)));
    assert!(a.drag_session.is_some());
    assert!(b.drag_session.is_none(), "drag_session is per-window — b unaffected by a");
    b.drag_session = Some(sonic_app::tab_drag::DragSession::new(7, (9.0, 9.0)));
...
fn multi_window_owns_independent_drag_target() {
    ...
    a.drag_target = Some(sonic_app::tab_drag::DropTarget { window: fake_id, slot: 0 });
