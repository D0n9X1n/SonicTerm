//! Keyboard / mouse / IME input encoding helpers split out of
//! [`super::core`] for refactor PR 8b.
//!
//! These are pure free functions and a tiny enum — no `App` state. They are
//! re-exported from [`super`] (via `pub use core::*` in `mod.rs` plus the
//! `pub use` lines below) so existing call sites keep compiling unchanged.
//!
//! Pre-split location: `crates/sonic-app/src/app.rs` lines 3903–4015.
