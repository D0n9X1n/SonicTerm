//! GPU-side wrapper around [`sonic_text::glyph_atlas::GlyphAtlas`].
//!
...
//! the headless `sonic-text` crate, which does not depend on wgpu. The
//! GPU wrapper stays here in `sonic-shared` (later: `sonic-gpu`).

use sonic_text::glyph_atlas::{GlyphAtlas, BYTES_PER_PIXEL};
