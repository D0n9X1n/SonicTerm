...
//! public hook `mark_all_dirty()`, which is what `sonic-shared` calls
//! from its app loop when those events fire (see
//! `mark_all_panes_dirty` in `sonic-shared/src/app.rs`).
//!
...
// `sonic-shared` app loop honours is: after each fires, call
