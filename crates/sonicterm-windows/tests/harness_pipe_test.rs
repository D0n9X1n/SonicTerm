...
/// Spec test (5.a) — full e2e against the running exe. Ignored until
/// active-pane sender publication is wired so it doesn't trip CI with
/// a known no-op assertion. To run manually:
///
/// ```pwsh
/// cargo test -p sonicterm-windows --features harness --test
