...
//!
//! Mirrors the structure of `sonic_text::tests::row_glyph_cache` for
...
fn empty_cache_misses_then_inserts() {
    ...
    let key = row_quad_hash(0, 0, &row, 0, 8.0, 16.0, 0.0, 0.0, 640.0, 384.0, None);
...
fn same_row_same_key_hits() {
    ...
    let k1 = row_quad_hash(0, 5, &row, 0, 8.0, 16.0, 0.0, 0.0, 640.0, 384.0, None);
    let k2 = row_quad_hash(0, 5, &row, 0, 8.0, 16.0, 0.0, 0.0, 640.0, 384.0, None);
...
fn content_change_misses() {
    ...
    let k_a = row_quad_hash(0, 0, &row_a, 0, 8.0, 16.0, 0.0, 0.0, 640.0, 384.0, None);
    let k_b = row_quad_hash(0, 0, &row_b, 0, 8.0, 16.0, 0.0, 0.0, 640.0, 384.0, None);
...
fn selection_overlap_misses() {
    ...
    let k_none = row_quad_hash(0, 3, &row, 0, 8.0, 16.0, 0.0, 0.0, 640.0, 384.0, None);
    // Selection covering rows 2..=4 overlaps row 3.
    let k_sel =
        row_quad_hash(0, 3, &row, 0, 8.0, 16.0, 0.0, 0.0, 640.0, 384.0, Some((2, 0, 4, 79)));
...
fn style_rev_bump_misses() {
    ...
    let k_old = row_quad_hash(0, 1, &row, 7, 8.0, 16.0, 0.0, 0.0, 640.0, 384.0, None);
    let k_new = row_quad_hash(0, 1, &row, 8, 8.0, 16.0, 0.0, 0.0, 640.0, 384.0, None);
...
fn invalidate_row_abs_drops_one_entry() {
    ...
    let k = row_quad_hash(0, 0, &row, 0, 8.0, 16.0, 0.0, 0.0, 640.0, 384.0, None);
