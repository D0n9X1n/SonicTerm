...
impl GpuRenderer {
    ...
    async fn new_async(
        ...
        // B4: Mailbox drops superseded frames so newest input wins;
        // Fifo's no-tear is wasted on a terminal. Mailbox is the wgpu
        // analogue of `VK_PRESENT_MODE_MAILBOX_KHR`; falls back to Fifo
        // on backends that don't expose it (e.g. some older drivers,
        // browser surfaces).
