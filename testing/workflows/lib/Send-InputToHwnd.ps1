...
# Keys whose scancode needs the extended flag (bit 24 in lParam,
# KEYEVENTF_EXTENDEDKEY for SendInput).
$script:SonicExtendedVK = @(0x21,0x22,0x23,0x24,0x25,0x26,0x27,0x28,0x2D,0x2E,0x0D)
...
function Send-TextToHwnd {
  ...
  foreach ($ch in $Text.ToCharArray()) {
    ...
    if ($vkScan -eq -1) {
      # Fall back to WM_CHAR only (no VK info).
      [void][SonicInput]::PostMessage($Hwnd, [SonicInput]::WM_CHAR, [IntPtr][int][char]$ch, [IntPtr]1)
      continue
    }
    ...
    [void][SonicInput]::PostMessage($Hwnd, [SonicInput]::WM_KEYDOWN, [IntPtr]$vk, $lpDown)
    [void][SonicInput]::PostMessage($Hwnd, [SonicInput]::WM_CHAR,    [IntPtr][int][char]$ch, $lpDown)
    [void][SonicInput]::PostMessage($Hwnd, [SonicInput]::WM_KEYUP,   [IntPtr]$vk, $lpUp)
  }
}
...
function Send-ChordToHwnd {
  ...
  try {
    ...
    $ki.dwFlags = 0
    if ($script:SonicExtendedVK -contains $vk) {
