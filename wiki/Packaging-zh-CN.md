...
验证器的命令共享自启动起 420 秒的准入截止时间，并为挂载清理预留 75 秒。只有测量镜像
`hdiutil create` 的临时性 `Resource busy` 失败会重试，最多两次；挂载自身只执行一次，
包括发生 `Resource temporarily unavailable` 时。

挂载使用 local-gate 进程组监督器。挂载前，完整的 `hdiutil info -plist` 清单必须证明解析后的
镜像及私有挂载点均未挂载。命令成功本身不够：后续清单必须标识一个新镜像及其精确私有
挂载点/设备，验证器才会读取它。整盘和分区条目（包括未挂载的设备）都被保留；清单格式
错误、截断或污染、挂载冲突以及进程是否结束未知，都会明确失败。

原始监督日志和 `attachment-result.json` 在验证器状态目录中记录命令及镜像清单。失败后仅在
预算允许时于第 0、2、5 秒进行最多三次只读观察；跳过必需观察会失败，不能解释为没有挂载。
只有当前清单证明挂载为本次调用新建且命令预算足够时才尝试卸载，不处理归属不明或外部镜像。
后续清单必须确认其消失；即使卸载命令成功，跳过或失败的确认仍算失败。后续成功完成归属明确的
卸载和消失确认后，较早的观察错误仍单独保留。无法读取的镜像别名会阻止证明，包括清理阶段，
不会被假定为外部镜像。清理也失败时仍优先报告原始验证错误，不会终止共享磁盘镜像服务。

清单查询最多 3 秒，挂载最多 60 秒，卸载最多 20 秒。验证命令会在 75 秒挂载清理预留之外，
另留运行器的 10 秒超时清理余量。挂载相关命令准入计入 15 秒普通监督余量。首次清理查询
保护 35 秒供卸载使用；后续观察保护 55 秒供卸载和确认使用。即使确认预算不再足够，只要
卸载自身的 35 秒余量足够仍可执行。缓慢的进程组扫描可能超出这些余量，因此准入期限不是
严格的墙钟上限，workflow 超时仍是最外层限制。不保证挂载一定释放或逃离的辅助进程一定
清理完毕。进程组结束和有限次清单观察不能证明逃离进程组的辅助进程以后不会完成挂载。
