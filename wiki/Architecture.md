...
- Dropping `PtyHandle` starts bounded process and I/O teardown. Transfer code
  validates both endpoints before it removes a pane because an accidental drop
...
- 析构 `PtyHandle` 会开始有时限的进程与 I/O 清理。转移代码先验证源端和目标端，再移除
  窗格，因为误析构会终止子进程。
