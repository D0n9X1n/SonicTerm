...
以及 READONLY 限制都会显示原因。复制要求非空选区，且所选单元格及连接各行的自动换行仍然匹配；
解析器忙时暂时禁用复制，而不阻塞 UI。同值重绘保留有效选区，除非它改变了自动换行连接所选各行的位置。
...
**Rename Active Tab** 和 **Update Tab Color** 修改编辑器打开时处于活动状态的标签页，
即使按 Enter 前已切换到其他标签页也是如此。若该标签页已先关闭或移到其他窗口，Enter
只会关闭编辑器，不会重命名或改色任何标签页；关闭编辑器所在窗口也会关闭编辑器。名称
留空仍会恢复自动标题。
...
换行及去除首尾空白后超过 128 个 Unicode 标量值的名称会显示拒绝原因。
在 macOS 上，**编辑 > 粘贴**及 Cmd+V 也优先插入接收窗口的重命名编辑器，而非其搜索框，
并先于 READONLY 拒绝逻辑处理。输入法组合期间只接收粘贴操作，不插入文字；被拒绝或
不可读取的剪贴板内容绝不会转为终端输入。发往另一个窗口的粘贴不会修改此名称。编辑器始终
...
cell 或连接所选各行的自动换行已经变化，SonicTerm 会清除过期选区，并保持剪贴板不变。
