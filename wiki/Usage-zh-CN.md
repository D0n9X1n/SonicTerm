...
源 pane 不参与镜像投递，因此不会收到两份输入。READONLY 窗口不会接收广播输入。
请谨慎使用：输入会发送到每个接收 pane，按键编码遵循各 pane 协商的终端模式。

完整默认快捷键、action 名称和自定义格式见 [快捷键](Keybindings-zh-CN)。

### 回看滚动历史

使用鼠标滚轮、滚动条、滚动操作、提示符导航、搜索或复制模式回看历史时，视图顶部那一行会被
固定。命令持续输出时，即使历史已满、每行新输出都会淘汰最旧的一行，顶部仍显示同一段文字。
重新加载配置并调低 `terminal.scrollback` 时，固定的文字同样保持不变。若被固定的行本身已被
丢弃，视图移到仍保留的最旧一行；若已没有历史（例如历史被 `CSI 3 J` 清除，许多 `clear` 命令
会发送它），视图恢复跟随实时输出。

备用屏幕上的全屏程序显示它自己的屏幕。程序退出后，视图回到被固定的行，并按期间丢弃的历史
行数平移。固定的行随窗格一起移动到其它标签页和窗口。滚动到底部，或按 Enter 提交输入，会
恢复跟随实时输出。
...
搜索读取每个单元格保存的完整文本，包括组合符号和其它零宽字符。子串模式先把查询和每个单元格的
文本转换为 NFC，再在不区分大小写时转为小写，因此预组合的 `é` 与 `e` 后接 U+0301 可以互相
找到。规范化按单元格进行，匹配基于规范化后的文本：保存 `e` 后接 U+0301 的单元格经 NFC 后是
单个 `é`，因此单独的 `e` 或 U+0301 都找不到它。正则模式直接匹配原始字符，不规范化模式或文本；
包含组合符号的匹配会高亮承载该符号的单元格，从同一单元格开始的多个匹配只计一次。
...
在 macOS 上，接收操作的窗口拥有重命名编辑器时，**编辑 > 粘贴**和 Cmd+V 也遵循此规则。
名称输入框优先于下层搜索框及 READONLY 拒绝逻辑。被拒绝、为空或不可读取的剪贴板内容
不会转交搜索框、shell 或广播窗格；输入法正在组合文字时忽略粘贴。
在另一个窗口粘贴时，仍按该窗口自身的输入路径处理，不会修改留在原窗口的名称输入框。
...
拖动属于开始拖动的窗格。指针移到其它窗格、窗格间隙或窗口外时，选区只扩展到该窗格最近的
cell。如果 SonicTerm 为这次按下捕获选区快照时解析器正忙，或按在窗口重绘前已因调整大小而
移除的 cell 上，就不会开始新的选区，当前选区和窗格焦点保持不变。选区快照不会等待解析器；
此前读取终端鼠标模式时仍可能等待。切换标签页、关闭该窗格或标签页，或调整大小改变了该窗格的
行数或列数，都会结束这次拖动。
...
SonicTerm 会在复制前清除它；自动换行连接所选各行的位置发生变化时也会清除。
终端程序也可通过 OSC 52 的 `c` target 写入最多 512 KiB
的 UTF-8 文字。剪贴板读取/查询、格式错误的 Base64、其它 selection target 和超限写入
都会被忽略。

复制遵循终端记录的换行。因输出到达右边缘而接续上一行的行，会与上一行直接相连，
不插入换行符，换行处的空格也会保留，因此长命令、路径或 URL 会粘贴为一行。真正的
换行复制为换行符，该处以及选区末尾的行尾空格会被去除。若上一行被改写，或该行是由
换行控制符到达的，它就不再算作自动换行，而作为独立的一行复制。放不进最后一列的宽字符
会提前换行，并在该列留下空白；这个空白复制为一个空格。
...
粘贴会进入活动窗格。若该窗格是广播源，粘贴也会进入每个广播接收窗格。拖入的文件进入
所落窗口的活动窗格，而不是指针下方的窗格；若该窗格是广播源，也会进入它的广播接收窗格。
SonicTerm 为每个接收窗格分别编码粘贴内容。

粘贴的文字原样发送。若窗格中的程序开启了括号粘贴（bracketed paste），该窗格收到的文字
带括号粘贴标记；其它窗格收到普通文字。

同时拖入的多个文件会变成一行带引号、以空格分隔的路径。SonicTerm 不会添加 Enter，你可以
先检查这一行再运行。每个窗格按其启动 shell 为路径加引号：

| 窗格的启动 shell | 路径引号 |
| --- | --- |
| `sh`、`bash`、`zsh`、`dash`、`ksh` | POSIX shell 引号规则 |
| `pwsh`、`powershell` | PowerShell 单引号；路径中的单引号（包括弯引号）加倍 |
| `cmd` | 双引号 |
| 其它 shell，例如 `fish` | POSIX shell 引号规则 |

名称带不带 `.exe`、大小写如何都能识别。引号规则只看窗格的启动 shell：在其中再启动的
shell，或经 SSH 进入的 shell，都不会改变它。

为避免发送会被误读或被截断的输入，窗格会拒绝以下粘贴：

- 拖入的路径不是有效的 Unicode，或含控制字符；
- 对 `cmd`，拖入的路径含 `"`、`%` 或 `!`；
- 编码后超过 16 MiB 的粘贴，引号、空格和括号粘贴标记都计入。

只要有一个路径被拒绝，该窗格就拒绝整个拖放；一个窗格拒绝不影响其它窗格。粘贴或拖放
所在的窗口会显示一条警告：说明有多少个接收窗格拒绝了这次粘贴，列出每种原因及其影响的
窗格数；对超大粘贴，还会给出所需的大小和上限。警告从不显示路径或粘贴的文字。

READONLY 窗口中的粘贴和拖放不会向终端发送任何内容。无论窗口可写还是处于 READONLY，搜索打开时，
剪贴板粘贴都只由该窗口的搜索查询接收，绝不会发送给 shell 或广播窗格。如果活动窗格暂时不可用，
