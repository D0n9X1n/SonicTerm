...
`UserEvent::RequestRedraw(WindowId)`。每个窗格持有 `Arc<AtomicU64>` 输出代际和普通的已观察
代际，两者随 `PaneState` 转移。工作线程只有在非空批次完成解析器、媒体和宿主事件处理并返回
后才以 Release 增加代际。事件循环查找当前重绘编号，即使窗口隐藏或设备停止，也先运行该
窗口的命令维护，再标记一次 Output 原因。转移只修改共享目标，不替换输出代际。

每个窗口持有原因代际、输入即时权限、原始显示器周期、最近实际呈现时刻和抑制状态。
公开的 `last_render: Instant` 仍是唯一的上次尝试节奏时钟；`request_redraw(&self)` 仍只转发
原生请求。软件降级在决策时读取全局策略。收集器在两类锁之前的回调用 Acquire 读取窗格身份。
完成时只消费本窗口捕获的原因/输出代际，不消费兄弟窗口或稍后的发布。只有可见输出影响突发
节奏；隐藏代际可以仅通过原子读取观察，无需访问解析器/媒体状态。
...
期限折叠通过 `DueWork` 保留每项的窗口和类型，只服务已经到期的项目，并将同一窗口的多个
原因合并为一次原生请求。只有子窗口到期时不会重绘主窗口。主窗口光标闪烁以及主/子窗口的
滚动条和通知过期均保留显式归属；前台进程采样只重绘状态变化的窗口，包括主窗口。退出确认、
内存、指针移动、OSC 52 和 smoke 维护不会吞掉同时到期的重绘。没有期限时使用
`ControlFlow::Wait` 停住循环。

命令徽标保留现有阈值：非活动标签运行满六秒时出现，已完成命令的徽标保留三秒，活动标签
也会过期。每个定时项捕获窗口、稳定标签编号和命令身份；关闭、转移、状态替换或激活不会
消费另一标签的旧期限。加锁前的帧快照也会在渲染采样之前捕获徽标的未来变化；等待折叠
会在替换期限前消费已到期项。因此，渲染或无关事件期间跨过阈值仍会调度所属窗口，且不会
重新安排已过期的期限。每次变化只标记所属窗口的界面脏状态，并与在途请求合并。标签栏隐藏
时不增加徽标唤醒；受抑制的窗口保留待处理脏状态而不请求帧。不增加周期徽标心跳，只有内存
采样到期仍不会重绘。

Unchanged/Noop/NoPanes 会消费捕获的请求，但不声称确认网格，也不会因保留脏行而产生心跳。
CachedReblit 还会记录实际呈现。每次渲染器调用恰好更新一次 `last_render` 并消费捕获的输入
即时权限，包括失败和由呈现器拥有的重试。Timeout 由应用控制节奏，后端 Occluded 抑制帧并使用
仅 macOS 的低频可用性探测；图集及其它表面原因仍由渲染器拥有重试。原生遮挡不贡献帧期限，
可见性转换使保留帧键失效，见[渲染模式](Rendering-Modes-zh-CN)。设备停止报告通过不组帧的
一次性入口先于可见性/拓扑抑制执行；停止的设备代际不贡献帧期限。

两个窗口角色只收集验证后的可见帧源，拥有所有权的句柄通过普通借用活得比解析器保护对象
更久。不访问或复制隐藏图像列表；覆盖所有窗格的保留/计费遍历保持不变。先取得解析器保护对象，
再分别取得可见图像快照。任一可见锁不可用时会推迟完整帧，并将该窗口的
...
失败使用退出码 `16`，GPU 故障隔离失败使用 `17`，GPU 设备丢失失败使用 `18`，共享设备恢复
失败使用 `19`。其它阶段成功但原生 PTY 清理未完成时使用 `NativeTeardown`，退出码 `20`。
更早的冒烟失败保留原退出码。
