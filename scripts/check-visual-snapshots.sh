...
# render regression.
set -e
if ! cargo test -p sonicterm-shared --test visual_snapshot 2>&1; then
  echo ""
  echo "Visual snapshot drift detected. Two options:"
  echo "1. Intentional rendering change: UPDATE_SNAPSHOTS=1 cargo test -p sonicterm-shared --test visual_snapshot"
