...
def workspace(report, meta, base):
    """Write the input documents to a temporary directory and yield their paths."""
...
class NotMeasuredTests(unittest.TestCase):
    ...
    def test_files_without_instrumented_lines_are_not_measured(self):
        ...
        files = {**FILES, "crates/sonicterm-harfbuzz/src/lib.rs": (0, 0)}
        run = check(files, MEMBERS + ["sonicterm-harfbuzz"], baseline(FLOORS), CI_ENV)
        self.assertEqual(run.status, 1, run.stdout)
        line = row(run.stdout, "sonicterm-harfbuzz")
        self.assertIn("UNEXPLAINED", line)
